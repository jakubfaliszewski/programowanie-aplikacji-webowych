...
export class UI {
    ...
    renderNotes(notes: INote[], force: boolean = false) {
        // if (force) {
        //     const wrapper = document.getElementById('notes-other');
        //     wrapper.innerHTML = null;
        // }
        notes.forEach((note) => {
            const wrapper = note.pinned 
                ? document.getElementById('notes-pinned')
                : document.getElementById('notes-other');
            new Note(note, wrapper);
